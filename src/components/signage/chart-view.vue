...
g(:transform="`translate(${padding.left}, ${padding.top})`")
  ...
  g(v-for="it of refData" :class="[{'z-1': it.hover}, {'opacity-40': !it.hover && refData.some(it => it.hover)}]")
    g(v-show="it.hover")
      ...
      line(:x1="it.x" :x2="it.x" :y1="drawSize.height" :y2="it.y" stroke="#5c6069" opacity="0.7" stroke-width="3" class="animation-line")
    g(:transform="circleTranslate(it)")
      text(fill="#333" transform="translate(15, 4)" :class="['text-xs', {'opacity-50': !it.hover}]") {{it.title}}
      circle(v-show="it.hover" r="12" fill="#5c6069" opacity="0.8")
      circle(v-show="it.hover" r="9" fill="#000" opacity="0.8")
...
export default {
  ...
  setup(props) {
    ...
    const drawSize = computed(() => ({
      width: props.width - props.padding.left - props.padding.right ?? 0,
      height: props.height - props.padding.top - props.padding.bottom ?? 0,
    }))
